...
class CMAMETrainer:
    ...
    def _phase_noise(self, phase_idx: int) -> np.ndarray:
        ...
        debug_log("Phase noise generated", phase=phase_idx, path=noise_path, noise_vector=noise_vec.tolist())
        return noise_vec
    ...
    def _checkpoint(
        ...
        ckpt_dir = save_checkpoint_bundle(
            ...
            measure_labels=self.cfg.archive_measure_labels,
            best_params_filename=self.cfg.best_params_filename,
            phase_params_filename=self.cfg.phase_best_params_filename,
            reward_curve_filename=self.cfg.reward_curve_filename,
        )
        if self.cfg.allow_save_best_model_video and phase_best_solution is not None:
    ...
    def train(self) -> tuple[Optional[np.ndarray], int, History]:
        ...
        worker_count = self.cfg.workers if self.cfg.workers > 0 else (os.cpu_count() or 1)
        debug_log("Starting Dask client", workers=worker_count)
        client = Client(n_workers=worker_count, threads_per_worker=1)
        print(f"[CMA-ME] active workers: {worker_count}")
        evaluate = partial(
            simulate_policy,
            env_id=self.cfg.env_id,
            seed=self.cfg.seed,
